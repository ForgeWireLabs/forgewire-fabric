...
if (-not $baseCommit) {
    ...
    # Fall back: compare current subtree tree hash against mirror HEAD tree hash.
    $monorepoTree = git -C $MonorepoPath rev-parse "HEAD:" 2>&1
    ...
    Fail "Mirror tree ($mirrorTree) diverges from monorepo subtree ($monorepoTree). Run a sync commit."
}
...
$monorepoTreeAtBase = git -C $MonorepoPath rev-parse "${baseCommit}:" 2>&1
if ($LASTEXITCODE -ne 0) {
    Fail "Base commit $baseCommit not found in monorepo. History may have been rewritten." 2
}
...
git -C $MonorepoPath diff --name-status "${baseCommit}:" "HEAD:" 2>&1 | Select-Object -First 40
Write-Host ""
Fail "Mirror diverges from monorepo subtree at $($baseCommit.Substring(0,12)). " + `
     "Re-sync the mirror with: git subtree push --prefix=forgewire-fabric <mirror-remote> main"
