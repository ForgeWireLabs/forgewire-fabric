...
.DESCRIPTION
    ...
    What gets installed on EVERY node:
      ...
      3. Realm identity  - Interactive Master (operator) account setup (114D
                           D.1/D.2), hub node only, one-time. Prompts for a
                           realm name, username, and password, then mints the
                           realm's cryptographic identity and your admin
                           account. Recovery codes are shown once. Skippable
                           with -SkipGenesis (unattended/non-interactive runs).
      4. Runner         - Command runner background service (NSSM)
      5. Watchdogs      - Hub + runner liveness probes (scheduled tasks)
      6. VSIX           - VS Code extension for the Fabric sidebar
      7. MCP (optional) - Dispatcher MCP registration (requires Python in venv)
...
.PARAMETER SkipGenesis
    Skip the interactive realm/Master-account setup prompt (hub node only).
    Use for unattended/scripted installs, or non-interactive shells (e.g. SSH
    without a PTY) where Read-Host cannot prompt. The realm can be established
    later by re-running this installer interactively, or by calling
    POST <hub-url>/setup/complete directly.
...
if (-not $principal.IsInRole([System.Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    # -NoExit: the elevated re-launch opens a brand new console window. Without
    # this, that window closes itself the instant the script exits -- on
    # success *or* on an uncaught error (this script sets
    # $ErrorActionPreference = "Stop", so any install-step failure throws) --
    # so a failure a re-run hits (e.g. a service already installed and running)
    # would flash and vanish before the operator could read it. -NoExit leaves
    # the window open with a live prompt so the operator always sees the
    # outcome, not just a window that appeared to do nothing.
    $forwarded = @('-NoProfile','-NoExit','-ExecutionPolicy','Bypass','-File',$PSCommandPath)
    foreach ($k in $PSBoundParameters.Keys) {
        $v = $PSBoundParameters[$k]
        if ($v -is [switch]) { if ($v.IsPresent) { $forwarded += "-$k" } }
        else                 { $forwarded += "-$k"; $forwarded += $v }
    }
    Write-Host "Requesting elevation -- a new window will open and stay open; check it for output."
    $proc = Start-Process -FilePath $shellExe -Verb RunAs -PassThru -ArgumentList $forwarded
    Write-Host "Elevated installer window launched (PID $($proc.Id)). This window can be closed."
    exit 0
}
...
Write-Host ""
Write-Host "-- 1/7 -- rqlite (Raft member on every node)..." -ForegroundColor Cyan
...
Write-Host "-- 1/7 -- rqlite OK" -ForegroundColor Green
...
Write-Host ""
Write-Host "-- 2/7 -- Hub (Rust binary, rqlite backend) on this node..." -ForegroundColor Cyan
...
Write-Host "-- 2/7 -- Hub OK" -ForegroundColor Green

# == STEP 3 - Realm identity / Master account (hub node only, one-time) ═══════
# Establishes the fabric's cryptographic identity and the first human account
# (114D D.1/D.2) -- the step the old bearer-token-only model never had.
# Idempotent and safe to re-run: does nothing once a realm already exists.
if ($isHubNode -and -not $SkipGenesis) {
    Write-Host ""
    Write-Host "-- 3/7 -- Realm identity..." -ForegroundColor Cyan
    $setupStatus = $null
    try {
        $setupStatus = Invoke-RestMethod -Uri "$effectiveHubUrl/setup/status" -TimeoutSec 5
    } catch {
        Write-Warning "Could not reach $effectiveHubUrl/setup/status: $($_.Exception.Message)"
    }

    if ($setupStatus -and $setupStatus.realm_established) {
        Write-Host "  Realm already established -- skipping." -ForegroundColor DarkGray
    } elseif ($setupStatus -and $setupStatus.bootstrap_open -and -not ([Console]::IsInputRedirected)) {
        Write-Host ""
        Write-Host "  No realm exists yet on this hub. Let's create your Master (operator) account." -ForegroundColor Yellow
        Write-Host "  This is a one-time step -- it establishes the realm's cryptographic identity" -ForegroundColor DarkGray
        Write-Host "  and your admin account. Recovery codes are shown once; save them." -ForegroundColor DarkGray
        Write-Host ""

        $realmName = Read-Host "  Realm name [ForgeWire Fabric]"
        if (-not $realmName) { $realmName = "ForgeWire Fabric" }

        $masterUsername = ""
        while (-not $masterUsername) { $masterUsername = Read-Host "  Your username" }

        $masterDisplayName = Read-Host "  Your display name [$masterUsername]"
        if (-not $masterDisplayName) { $masterDisplayName = $masterUsername }

        $masterPassword = $null
        while (-not $masterPassword) {
            $secure1 = Read-Host "  Choose a password (12+ characters recommended)" -AsSecureString
            ...
            $masterPassword = $plain1
        }

        # Loopback-origin ceremonies (114D sec 5): the realm's rp_id defaults to
        # "localhost", so its registered origins must be the literal hostname
        # localhost on this hub's own port, plus the Tauri webview's origin.
        $genesisBody = @{
            realm_name   = $realmName
            origins      = @("http://localhost:$HubPort/", "http://tauri.localhost/")
        ...
        try {
            $genesisResult = Invoke-RestMethod -Method Post -Uri "$effectiveHubUrl/setup/complete" `
                -ContentType "application/json" -Body $genesisBody -TimeoutSec 20
            Write-Host ""
            Write-Host "  Realm established. Master account: $($genesisResult.account.username)" -ForegroundColor Green
            Write-Host ""
            Write-Host "  ┌─────────────────────────────────────────────────────────┐" -ForegroundColor Yellow
            Write-Host "  │  RECOVERY CODES -- shown once. Save these somewhere safe. │" -ForegroundColor Yellow
            Write-Host "  │  If you ever lose your password, these are the way back.  │" -ForegroundColor Yellow
            Write-Host "  └─────────────────────────────────────────────────────────┘" -ForegroundColor Yellow
            foreach ($code in $genesisResult.recovery_codes) {
                Write-Host "    $code" -ForegroundColor White
            }
            Write-Host ""
            Read-Host "  Press Enter once you have saved these recovery codes" | Out-Null
        } catch {
            $body = $null
            if ($_.Exception.Response) {
                try { $body = (New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())).ReadToEnd() } catch {}
            }
            Write-Warning "Genesis setup failed: $($_.Exception.Message)$(if ($body) { " -- $body" })"
            Write-Warning "Re-run this installer to try again, or complete it manually: POST $effectiveHubUrl/setup/complete"
        } finally {
            # Best-effort: PowerShell strings are immutable, so this dereferences
            # rather than zeroes the underlying memory -- a known language
            # limitation, not something this script can fully close.
            $masterPassword = $null
            $genesisBody = $null
        }
    } elseif ($setupStatus -and $setupStatus.bootstrap_open) {
        Write-Warning "Realm setup is open but this session cannot prompt interactively (input is redirected)."
        Write-Warning "Re-run interactively, pass -SkipGenesis to silence this, or POST $effectiveHubUrl/setup/complete directly."
    } else {
        Write-Warning "Could not determine setup status -- skipping genesis. Check the hub manually: $effectiveHubUrl/setup/status"
    }
    Write-Host "-- 3/7 -- Realm identity OK" -ForegroundColor Green
} elseif ($isHubNode -and $SkipGenesis) {
    Write-Host ""
    Write-Host "-- 3/7 -- Realm identity skipped (-SkipGenesis)" -ForegroundColor DarkGray
}

# == STEP 4 - Runner (Rust binary) ══════════════════════════════════════════════
Write-Host ""
Write-Host "-- 4/7 -- Runner (Rust binary)..." -ForegroundColor Cyan
...
& $runnerInstaller `
    ...
    -Tags         $runnerTags `
    -NoWatchdog  # watchdog installed in step 4
Write-Host "-- 4/7 -- Runner OK" -ForegroundColor Green

# == STEP 5 - Watchdogs ══════════════════════════════════════════════════════════
Write-Host ""
Write-Host "-- 5/7 -- Watchdogs..." -ForegroundColor Cyan
...
Write-Host "-- 5/7 -- Watchdogs OK" -ForegroundColor Green

# == STEP 6 - VSIX ══════════════════════════════════════════════════════════════
if (-not $SkipVsix) {
    Write-Host ""
    Write-Host "-- 6/7 -- VSIX..." -ForegroundColor Cyan
    ...
    Write-Host "-- 6/7 -- VSIX OK" -ForegroundColor Green
} else {
    Write-Host "-- 6/7 -- VSIX skipped (-SkipVsix)" -ForegroundColor DarkGray
}

# == STEP 7 - MCP + host roles (optional, requires Python) ════════════════════
Write-Host ""
Write-Host "-- 7/7 -- MCP + host roles..." -ForegroundColor Cyan
...
# == Registered operator overlays ═══════════════════════════════════════════════
# Operator state lives outside $DataDir so a normal uninstall/reinstall cannot
# erase consumer-owned services or identities. Cached artifacts make replay
# independent of Cargo and of an existing target/release directory.
$overlayReplay = Join-Path $PSScriptRoot 'replay-operator-overlays.ps1'
if (Test-Path -LiteralPath $overlayReplay) {
    & $overlayReplay -FabricRoot $FabricRoot -StartServices
    if (-not $?) { throw 'Registered operator overlay replay failed.' }
}
...
Write-Host "  The hub bearer token gates machine/automation access (agents, runners," -ForegroundColor DarkGray
Write-Host "  dispatchers). Anyone with it can dispatch tasks to this cluster." -ForegroundColor DarkGray
Write-Host "  Rotate it with: forgewire-fabric token rotate" -ForegroundColor DarkGray
Write-Host "  Human/operator access is separate: your Master account (realm identity," -ForegroundColor DarkGray
Write-Host "  114D D.1/D.2), set up interactively above unless -SkipGenesis was passed." -ForegroundColor DarkGray
Write-Host "  Sign in with it via the Desktop app or VS Code extension." -ForegroundColor DarkGray
