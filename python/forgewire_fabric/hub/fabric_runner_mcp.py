"""MCP server (Fabric runner side) — agent claim path via /tasks/claim-fabric.

Replaces ``runner_mcp.py`` for agent-kind runners.  The existing ``runner_mcp``
module is a deprecated shim that re-exports from here.

Compared to ``runner_mcp``:
* ``claim_next_task`` routes to ``POST /tasks/claim-fabric`` (not ``claim-v2``).
* The claimed task record's ``dispatch`` field (skill | tool | prompt) is
  surfaced verbatim in the returned dict so the host agent (Claude Code, VSCode,
  etc.) can branch on the typed dispatch.
* Two new tools: ``mcp_manifest_refresh`` and ``agent_self_describe``.
* Server name advertised to MCP clients: ``forgewire-fabric-runner``.

All other tools (``mark_running``, ``report_*``, ``post_note``, ``read_notes``,
``get_task``, ``request_drain``, ``self_update``, ``runner_identity``) are
identical to ``runner_mcp`` and re-use the same implementations.

Env knobs: same set as ``runner_mcp`` — see that module's docstring.
"""
...
def _register_tools(registry: ToolRegistry, session: RunnerSession) -> None:
    ...
    async def claim_next_task(_args: dict[str, Any]) -> dict[str, Any]:
        ...
        return await client.claim_task_fabric(session.claim_payload())

    registry.register(
        name="claim_next_task",
        description=(
            "Claim the highest-priority queued agent task that matches this runner's "
            "capabilities (scope prefixes, tools, tags, tenant, repo state). "
            "Routes to /tasks/claim-fabric. Returns ``{task: <record|null>, info: {...}}`` "
            "where ``task.dispatch`` is one of ``skill | tool | prompt`` and "
            "``task.skill`` / ``task.tool`` / ``task.prompt`` carry the typed payload."
        ),
    ...
    async def mcp_manifest_refresh(_args: dict[str, Any]) -> dict[str, Any]:
        """Force a re-introspection of connected MCP servers without restarting."""
        new_manifest = build_mcp_manifest()
        new_hash = manifest_hash(new_manifest)
        changed = new_hash != session._mcp_manifest_hash
        session._mcp_manifest = new_manifest
        session._mcp_manifest_hash = new_hash
        if changed:
            LOGGER.info("mcp_manifest_refresh: manifest changed; re-registering")
            await _register_with_retries(session)
        return {
            "changed": changed,
    ...
    registry.register(
        name="mcp_manifest_refresh",
        description=(
            "Force a re-introspection of all connected MCP servers and rebuild "
            "the manifest. If the manifest changed, re-registers with the hub so "
            "the capability index is updated immediately (without waiting for the "
            "next heartbeat cycle)."
        ),
        input_schema={"type": "object", "properties": {}},
        handler=mcp_manifest_refresh,
    )

    async def agent_self_describe(_args: dict[str, Any]) -> dict[str, Any]:
        """Return the current MCP manifest as seen by the hub."""
        return {
            "runner_id": session.runner_id,
            "agent_type": os.environ.get("FORGEWIRE_RUNNER_AGENT_TYPE") or "claude-code",
            "kinds": ["agent"],
            "manifest": session._mcp_manifest,
            "manifest_hash": session._mcp_manifest_hash,
        }

    registry.register(
        name="agent_self_describe",
        description=(
            "Return this agent runner's current MCP manifest: the full set of "
            "tools, resources, and prompts advertised to the hub's capability "
            "index. Useful for verifying what capabilities are visible to "
            "skill/tool dispatch routing."
        ),
        input_schema={"type": "object", "properties": {}},
        handler=agent_self_describe,
    )
