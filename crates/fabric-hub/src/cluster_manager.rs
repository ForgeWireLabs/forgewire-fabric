...
async fn trigger_snapshot(base_url: &str) {
    let client = reqwest::Client::builder()
        ...
        .unwrap_or_default();
    match client
        .post(format!("{base_url}/snapshot?wait=true"))
        .send()
        .await
    {
        Ok(r) if r.status().is_success() => info!("rqlite snapshot complete — Raft log compacted"),
        ...
        TopologyAction::Promote(ids) => {
            ...
            for id in &ids {
                if let Some(node) = nodes.iter().find(|n| &n.id == id) {
                    info!(
                        "promoting {} to voter ({}-node cluster, full quorum)",
                        id,
                        nodes.len()
                    );
            ...
            if !promoted.is_empty() {
                ...
                record_suffrage_action(
                    base_url,
                    "promote-to-voter unsupported on this rqlite (needs node restart)",
                )
                .await;
                Ok(None)
            } else {
...
fn chrono_now_iso() -> String {
    ...
    format!(
        "{year:04}-{:02}-{:02}T{hours:02}:{mins:02}:{secs_in_min:02}Z",
        month + 1,
        days + 1
    )
}
...
async fn maybe_transfer_leadership(base_url: &str, nodes: &[RqliteNode]) -> anyhow::Result<()> {
    ...
    if total < 3 {
        return Ok(());
    } // only relevant for full-quorum clusters

    let Some(preferred_id) = get_preferred_leader(base_url).await else {
        return Ok(()); // no preference recorded yet
    };
    ...
    match (current_leader, preferred_node) {
        (Some(leader), Some(preferred))
            if leader.id != preferred_id && preferred.reachable && preferred.voter =>
        {
            info!(
                from = %leader.id,
                to   = %preferred_id,
...
mod tests {
    ...
    fn single_node_is_stable() {
        assert_eq!(
            decide_topology(&[node("a", true, true, true)]),
            TopologyAction::Nothing
        );
    }
    ...
    fn two_voters_demotes_the_non_leader() {
        ...
        let nodes = [
            node("leader", true, true, true),
            node("standby", true, false, true),
        ];
        assert_eq!(
            decide_topology(&nodes),
            TopologyAction::Demote("standby".to_owned())
        );
    }
    ...
    fn two_nodes_leaderless_emergency_promotes() {
        let nodes = [
            node("a", false, false, true),
            node("b", false, false, false),
        ];
        assert_eq!(
            decide_topology(&nodes),
            TopologyAction::EmergencyPromote("a".to_owned())
        );
    }
