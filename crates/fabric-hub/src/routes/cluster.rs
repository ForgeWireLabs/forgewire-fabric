...
owned_router! {
    pub fn router, ROUTES {
        "GET" get "/cluster/health" => cluster_health;
        "GET" get "/hosts" => list_hosts;
    }
}

owned_router! {
    pub fn audit_router, AUDIT_ROUTES {
        "GET" get "/audit/day/{day}" => audit_day;
    }
}

// ── GET /cluster/health ────────────────────────────────────────────────────

pub async fn cluster_health(State(state): State<Arc<HubState>>) -> Json<Value> {
    let rqlite = if state.backend.starts_with("rqlite") {
        let host =
            std::env::var("FORGEWIRE_HUB_RQLITE_HOST").unwrap_or_else(|_| "127.0.0.1".into());
        let port: u16 = std::env::var("FORGEWIRE_HUB_RQLITE_PORT")
            .ok()
...
pub async fn list_hosts(
    ...
    let dispatchers = state
        .store
        .list_dispatchers()
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let host_roles = state
        .store
        .list_host_roles()
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    for runner in &runners {
        ...
        if let Some(obj) = rv.as_object_mut() {
            ...
            obj.insert(
                "host_alias".into(),
                json!(host_aliases.get(&h).cloned().unwrap_or_default()),
            );
        }
    ...
    let hosts: Vec<Value> = hostnames
        ...
        .map(|hostname| {
            ...
            let runner_alias = host_runners
                ...
                .unwrap_or_default();
            let label = if !host_alias.is_empty() {
                host_alias.clone()
            } else {
                runner_alias
            };
            ...
            let hub_head = role_summary(
                is_active_hub,
                if is_active_hub { "active" } else { "standby" },
                "active_hub",
                Some(hub_url.clone()),
                vec![],
                vec![],
            ...
            let control = role_summary(
                is_active_hub,
                if is_active_hub { "master" } else { "slave" },
                "active_hub",
                None,
                vec![],
                vec![],
            ...
            let dispatcher_ids: Vec<String> = host_dispatchers
                .iter()
                .filter_map(|d| d["dispatcher_id"].as_str().map(|s| s.to_owned()))
                .collect();
            let dispatch_row = stored.get("dispatch");
            let dispatch_enabled = !host_dispatchers.is_empty()
                || dispatch_row
                    .map(|r| r["enabled"].as_bool().unwrap_or(false))
                    .unwrap_or(false);
            ...
            let dispatch_meta = dispatch_row
                .and_then(|r| r.get("metadata"))
                .cloned()
                .unwrap_or(json!({}));
            let dispatch_updated = dispatch_row
                .and_then(|r| r["updated_at"].as_str())
                .map(|s| s.to_owned());
            let dispatch_source = if !host_dispatchers.is_empty() {
                "dispatcher_registry"
            } else {
                "derived"
            };
            let dispatch = role_summary(
                dispatch_enabled,
                &dispatch_status,
                dispatch_source,
                None,
            ...
            let command_runner =
                build_runner_role(&host_runners, stored.get("command_runner"), "command");
            let agent_runner =
                build_runner_role(&host_runners, stored.get("agent_runner"), "agent");
...
// Pure JSON-shape formatting helper: every parameter is moved directly into
// the `json!` object below, so taking them by value (not by reference) is
// the natural, ergonomic shape at this function's few call sites.
...
fn build_runner_role(runners: &[Value], role_row: Option<&Value>, kind: &str) -> Value {
    let role_runners: Vec<&Value> = runners
        .iter()
        .filter(|r| runner_kind_from_tags(&r["tags"]) == kind)
        .collect();
    let enabled = !role_runners.is_empty()
        || role_row
            .map(|r| r["enabled"].as_bool().unwrap_or(false))
            .unwrap_or(false);
    ...
    let source = if !role_runners.is_empty() {
        "runner_heartbeat"
    } else if role_row.is_some() {
        "host_roles"
    } else {
        "derived"
    };
    let runner_ids: Vec<String> = role_runners
        .iter()
...
fn runner_rollup_status(runners: &[&Value]) -> String {
    let states: Vec<&str> = runners.iter().filter_map(|r| r["state"].as_str()).collect();
    if states.contains(&"online") {
        return "online".into();
    }
    if states.contains(&"degraded") {
        return "degraded".into();
    }
    if states.contains(&"draining") {
        return "draining".into();
    }
    "offline".into()
}
...
pub async fn audit_day(
    ...
    let events = state
        ...
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let (verified, err) = state
        .store
        .verify_audit_chain(&events)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(
        json!({ "day": day, "events": events, "verified": verified, "error": err }),
    ))
}
